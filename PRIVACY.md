...
- Fleet caches contain session, schedule, host-health, attention, and embedded-tab descriptor metadata only. When Automatic coding-session titles is enabled, they may also contain one bounded host-derived session label; the setting can be disabled to stop requesting and purge those labels. Complete prompts, responses, transcripts, terminal bytes, question answers, and staged image bytes are excluded.
