...
function renderWidgetSettingsSection(): string {
  ...
  return `
    <section class="settings-section"><h2>Widget</h2><div class="settings-grid">
      ...
      <label>Codex row order<select data-setting="codexSortMode"><option value="highestAverageUse" ${settingsDraft.codexSortMode === 'highestAverageUse' ? 'selected' : ''}>Highest average use</option><option value="profileOrder" ${settingsDraft.codexSortMode === 'profileOrder' ? 'selected' : ''}>Configured order</option></select></label>
...
function updateSettingsFromInput(input: HTMLInputElement | HTMLSelectElement): void {
  ...
  if (setting === 'codexSortMode') settingsDraft.codexSortMode = input.value === 'profileOrder' ? 'profileOrder' : 'highestAverageUse';
